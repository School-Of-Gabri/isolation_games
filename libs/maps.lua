...
function maps.funny_grid(width, height)
  local grid = {}
  local grid_size = 50
  local max_x_offset = 20
  local max_y_offset = 20


  for x_pos = 0, width, grid_size do
    for y_pos = 0, height, grid_size do
      ...
      table.insert(grid, maps.mesh(x_pos, y_pos, grid_size, grid_size, curx_rnd, cury_rnd))
    end
  end
  return grid
end

function maps.grid(width, height)
  local grid = {}
  local grid_size = 50
  ...
  for x_pos = 0, width, grid_size do
    for y_pos = 0, height, grid_size do
      local tile = {
        TL = {x = x_pos, y = y_pos},
        BL = {x = x_pos + grid_size, y = y_pos},
        TR = {x = x_pos, y = y_pos + grid_size},
        BR = {x = x_pos + grid_size, y = y_pos + grid_size}
      }
